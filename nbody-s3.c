...
void compute_n_bodies(nbody* info) {
    size_t output_steps = info->output_steps, output_rows = info->output_rows, 
    ...
    for (size_t time = 0; time < num_steps; time++) {
        ...
        double accelerations[num_bodies][3];  // 3 acceleration values per body
        memset(accelerations, 0, sizeof(accelerations));  // initialize all values to 0
        for (size_t i = 0; i < num_bodies; i++) {
            ...
            for (size_t j = 0; j < i; j++) {
                ...
                // Add accelerations for F_ij
                accelerations[i][0] += x_diff * mass_j_force;
                accelerations[i][1] += y_diff * mass_j_force;
                accelerations[i][2] += z_diff * mass_j_force;

                // Subtract accelerations for F_ji
                accelerations[j][0] -= x_diff * mass_i_force;
                accelerations[j][1] -= y_diff * mass_i_force;
                accelerations[j][2] -= z_diff * mass_i_force;
            }
        }

        // Store every body's new positions based on their accelerations
        for (size_t i = 0; i < num_bodies; i++)
        {
            size_t x_i = i * 3, y_i = x_i + 1, z_i = y_i + 1;
            double x = curr_pos[x_i], y = curr_pos[y_i], z = curr_pos[z_i];
            ...
            double velocity_x = curr_vel[x_i] += accelerations[i][0] * time_step;
            double velocity_y = curr_vel[y_i] += accelerations[i][1] * time_step;
            double velocity_z = curr_vel[z_i] += accelerations[i][2] * time_step;
...
int main(int argc, const char* argv[]) {
    ...
    Matrix* output = matrix_zeros(num_outputs, output_rows);
